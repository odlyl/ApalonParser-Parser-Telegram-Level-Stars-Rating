...
Скрипт для поиска пользователей в Telegram-чате по уровню. Если пользователь подходит под фильтры, бот отправляет администратору карточку с профилем.
...
- Фильтр по минимальному и максимальному уровню.
...
Открой `config/__init__.py` и заполни свои данные:
...
4. Пропускает ботов.
5. Получает уровень и дополнительные параметры пользователя.
6. Сравнивает пользователя с фильтрами из `config/__init__.py`.
